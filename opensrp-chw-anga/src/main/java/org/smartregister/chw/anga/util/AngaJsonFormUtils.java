...
import static org.smartregister.chw.anga.util.Constants.ENCOUNTER_TYPE;
import static org.smartregister.chw.anga.util.Constants.STEP_ONE;
import static org.smartregister.chw.anga.util.Constants.STEP_TWO;

public class AngaJsonFormUtils extends org.smartregister.util.JsonFormUtils {
    public static final String METADATA = "metadata";

    public static Triple<Boolean, JSONObject, JSONArray> validateParameters(String jsonString) {

        JSONObject jsonForm = toJSONObject(jsonString);
        JSONArray fields = angaFormFields(jsonForm);
    ...
    public static JSONArray angaFormFields(JSONObject jsonForm) {
        try {
    ...
    public static Event processJsonForm(AllSharedPreferences allSharedPreferences, String
        ...
        if (Constants.EVENT_TYPE.ANGA_CONFIRMATION.equals(encounter_type)) {
            encounter_type = Constants.TABLES.ANGA_CONFIRMATION;
        } else if (Constants.EVENT_TYPE.ANGA_FOLLOW_UP_VISIT.equals(encounter_type)) {
            encounter_type = Constants.TABLES.ANGA_FOLLOW_UP;
        }
    ...
    protected static FormTag formTag(AllSharedPreferences allSharedPreferences) {
        ...
        formTag.appVersion = AngaLibrary.getInstance().getApplicationVersion();
        formTag.databaseVersion = AngaLibrary.getInstance().getDatabaseVersion();
        return formTag;
    }
    ...
    public static void tagEvent(AllSharedPreferences allSharedPreferences, Event event) {
        ...
        event.setClientApplicationVersion(AngaLibrary.getInstance().getApplicationVersion());
        event.setClientDatabaseVersion(AngaLibrary.getInstance().getDatabaseVersion());
    }
    ...
    public static JSONObject getFormAsJson(String formName) throws Exception {
        return FormUtils.getInstance(AngaLibrary.getInstance().context().applicationContext()).getFormJson(formName);
    }
